// Use CDN script directly instead of import
...
// Use global emojiMart.Picker instead of importing
...
function backToChatList() {
    document.querySelector('#chatting-page-mobile').classList.add('hidden');
    document.querySelector('.chat-list-mobile').classList.remove('hidden');
}

function sendMessage(event) {
    event.preventDefault();
    const isMobile = !event.target.id.includes('pc');
    const receiverId = document.querySelector(`#receiver-id-${isMobile ? 'mobile' : 'pc'}`).value;
    const groupId = document.querySelector(`#group-id-${isMobile ? 'mobile' : 'pc'}`).value;
    const content = document.querySelector(`#message-content-${isMobile ? 'mobile' : 'pc'}`).value.trim();
    if (!content) return;
    ...
    document.querySelector(`#message-content-${isMobile ? 'mobile' : 'pc'}`).value = '';
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.message.sent').forEach(message => {
        message.addEventListener('mouseenter', () => {
            message.querySelector('.message-actions').classList.remove('opacity-0');
        });
        message.addEventListener('mouseleave', () => {
            message.querySelector('.message-actions').classList.add('opacity-0');
        });
    });
});
